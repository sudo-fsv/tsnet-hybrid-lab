...
## Summary 
The goal of this lab is to evaluate the following:
...
* Leverage iperf3 to test bandwidth when Tailscale connects over peering instead of IGW -- WIP.
...
**Note:** This repository first commit was 100% generated by GitHub CoPilot in less than 15m with less than 5 prompts detailing designs and the separate modules: secrets, runner and the lab itself. It took me another day of work to review code, test, troubleshoot and fix bugs before I could finally leverage the lab main goal to validate networks stats of Tailscale in AWS hybrid environment over hard and easy NATs.


## LAB Topology
This is the original version generated by AI and redered by **Graphviz DOT** .
It is kept here for comparison purposes :)
...
### Folders created:
- `terraform/runner` — Terraform to launch a t3.micro EC2 instance (basic SG + outputs) to serve as self-hosted GitActions runner. 
- `terraform/github-secrets` — Terraform to create GitHub Actions repository secrets to store AWS and Tailscale secrets that will be safely consumed by pipeline. This workspace implies local Terraform apply.
...
### Notes and caveats:
- The EKS module used depends on community modules (`terraform-aws-modules/eks/aws`) and the VPC module (`terraform-aws-modules/vpc/aws`). Run `terraform init` to fetch them. This are AWS supported modules that can simplify the code structure, but as modules themselves they are usually trying to do too much, too complex and are shipped with bugs.
...
- The runner EC2 will require admin permission on AWS VPC, EC2, EKS, KMS, SSM, IAM and Cloudwatch logs.
...
1. GitHub CoPilot was able to generate the entire setup from less than 5 prompts in less than 15 minutes, but where context was not clear it complicated things such the proper method to install Tailscale using a Helm Terraform resource does not seem to be wildly adopted, so it missed important dependencies.
2. The AI assistant also got confused with proper variable scaping/interpolation between Terraform *user_data* scripts and Bash code. Bugs!
...
4. The overall documentation, comments and diagram /topology as IaC was quite resourceful and helpful.
5. If AI assistant gets lost on why the EKS is not finishing a deployment or exposing a service due to lack of context, it might suggest troubleshooting via *kubectl*, which will often leads to rabbit holes and complex sub-optimal code changes that force us into a feedback loop of causing another problem while fixing the previous one. Breaking the loop, starting again with fresh code and providing context helped to solve the problem. 
...
3. Evaluate the impact on latency while using subnet routers
