...
output "hello_via_subnet_router_ip" {
  value = local.eks_node_ip != "" ? format("%s:%d", local.eks_node_ip, kubernetes_service_v1.hello_via_subnet_router.spec[0].port[0].node_port) : ""
  description = "IP:port for accessing the hello service via the nodePort on an EKS node"
}
